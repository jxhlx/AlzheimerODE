...
class Trainer:
    ...
    def evaluate_generation(self, save_metric_details=False, plot_density=False, ema=False):
        ...
        start = time.time()
        def sample(X_train, X_test, split):
            ...
            test_num, test_cat, test_target = split_num_cat_target(X_test, info, num_inverse, int_inverse, cat_inverse)
            test_df = recover_data(test_num, test_cat, test_target, info)
            ...
            if split == 'val':
                ...
                y = np.array(X_train['age'])
                X_num_shape = len([c for c in X_train.columns.tolist() if 'Node' in c])
                X_cat = np.stack([np.array(X_train[c]) for c in X_train.columns.tolist() if 'label' in c], 1)
            ...
            org_y = torch.from_numpy(y.astype(int)).to(device)
            max_iter = 10
            sample_iter = 0
            while len(remain_sample_id) > 0:
                ...
                gen_data = self.sample_synthetic(batch_size, ema=ema)
            ...
            os.makedirs(f'outputs/tabdiff/ATN/2', exist_ok=True)
            np.save(f'outputs/tabdiff/ATN/2/X_num_{split}', sample_gen.numpy())

        sample(self.dataset, self.test_dataset, split='val')
