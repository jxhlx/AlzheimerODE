...
parser.add_argument('--save', type=str, default='experiments/bad/', help="Path for save checkpoints")
parser.add_argument('--load', type=str, default=None, help="ID of the experiment to load for evaluation. If None, run a new experiment.")
parser.add_argument('-r', '--random-seed', type=int, default=1991, help="Random_seed")

parser.add_argument('--dataset', type=str, default='atn', help="Dataset to load. Available: physionet, activity, hopper, periodic")
...
if __name__ == '__main__':
	...
	if args.dataset == 'atn':
		...
		all_set = ATNDataset(
			path='data/ALL/train',
			json_file='data/TABLE_Destrieux.json',
			normalization=True,
			mix_age=True,
			device=device
		)
		...
		train_gen = utils.inf_generator(train_loader)
		test_gen = utils.inf_generator(test_loader)

		sample = test_set[0]
		input_dim = sample['observed_data'].shape[-1]

		data_obj = {
			"train_dataloader": train_gen,
			"test_dataloader": test_gen,
			"input_dim": input_dim,
	...
	if args.rnn_vae:
		...
		cond_dim = 5 if args.dataset == 'atn' else 0
		n_labels = data_obj['n_labels']
