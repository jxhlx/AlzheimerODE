...
def create_LatentODE_model(args, input_dim, z0_prior, obsrv_std, device, 
	classif_per_tp = False, n_labels = 1, condition_dim = 0):

	dim = args.latents
	total_latent_dim = args.latents + condition_dim
	...
	if args.poisson:
		...
		dim = total_latent_dim
		ode_func_net = utils.create_net(dim, dim,
			n_layers = args.gen_layers, n_units = args.units, nonlinear = nn.Tanh)
	...
	decoder = Decoder(total_latent_dim, gen_data_dim).to(device)

	diffeq_solver = DiffeqSolver(gen_data_dim, gen_ode_func, 'dopri5', total_latent_dim,
		odeint_rtol = 1e-3, odeint_atol = 1e-4, device = device)
