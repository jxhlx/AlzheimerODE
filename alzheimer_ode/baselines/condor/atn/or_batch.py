...
class OrdinalRegression():
    def __init__(self, args):
        self.df = pd.read_csv(os.path.join(args.dir, 'atn.csv'))
        self.age = self.df['age']
        self.label = self.df['label']
        self.data = self.df.iloc[:, -444:]
        self.data = round(self.data, 4) 
        self.x_columns = ['Node ' + str(i) for i in range(1, 445)]
        ...
        self.model = OrderedModel(self.label, self.df[['age_norm'] + self.x_columns], distr='logit', has_constant='skip')
