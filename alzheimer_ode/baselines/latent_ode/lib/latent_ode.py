...
class LatentODE(VAE_Baseline):
    ...
    def get_reconstruction(self, time_steps_to_predict, truth, truth_time_steps,
                           mask=None, n_traj_samples=1, run_backwards=True, mode=None, cond=None):
        ...
        if cond is not None and self.condition_dim > 0:
            cond_expanded = cond.unsqueeze(0).repeat(n_traj_samples, 1, 1)

            first_point_enc_aug = torch.cat([first_point_enc, cond_expanded], dim=-1)
        else:
        ...
        if self.use_poisson_proc:
            n_traj_samples, n_traj, n_dims = first_point_enc_aug.size()
            zeros = torch.zeros([n_traj_samples, n_traj, self.input_dim]).to(get_device(truth))
            first_point_enc_aug = torch.cat((first_point_enc_aug, zeros), -1)
    ...
    def sample_traj_from_prior(self, time_steps_to_predict, cond=None, n_traj_samples=1):
        # input_dim = starting_point.size()[-1]
        # starting_point = starting_point.view(1,1,input_dim)

        batch_size = cond.size(0) if cond is not None else 1

        # Sample z0 from prior
        starting_point_enc = self.z0_prior.sample([n_traj_samples, batch_size, self.latent_dim]).squeeze(-1)

        if cond is not None and self.condition_dim > 0:
            cond_expanded = cond.unsqueeze(0).repeat(n_traj_samples, 1, 1)
            starting_point_enc_aug = torch.cat([starting_point_enc, cond_expanded], dim=-1)
        else:
