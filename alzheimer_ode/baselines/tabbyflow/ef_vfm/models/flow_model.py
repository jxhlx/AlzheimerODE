...
class ExpVFM(torch.nn.Module):
    ...
    def sample(self, num_samples):
        ...
        if sum(self.num_classes) != 0:
            ...
            #     idx += val
            #     assert val >= sample[
            #         :, i].max() >= 0, f"Sampled value {sample[:, i].max()} is out of range for categorical feature {i} with {val} classes."

            idx = self.num_numerical_features
            for i, val in enumerate(self.num_classes):
                cat_logits = out[:, idx:idx + val]
                sample[:, self.num_numerical_features + i] = torch.argmax(cat_logits, dim=1)
                idx += val
                assert val >= sample[:, self.num_numerical_features + i].max() >= 0, (
                    f"Sampled value {sample[:, self.num_numerical_features + i].max()} is out of range "
                    f"for categorical feature {i} with {val} classes."
                )
