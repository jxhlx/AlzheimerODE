...
class Trainer1D(object):
    def __init__(
        ...
        *,
        ...
        train_dl = DataLoader(train_dataset, batch_size=73, collate_fn=train_dataset.collate_fn, shuffle = True, pin_memory = True, num_workers = cpu_count())
        self.test_dl = cycle(train_dl)
        self.test_size = 73
    ...
    def metrics(self, all_samples, test_x):
        ...
        if (torch.min(all_samples) < -1e+20) or (torch.max(all_samples) > 1e+20):
    ...
    def train(self):
        ...
        if self.warmup == True:
            ...
            pth = os.path.join('checkpoints/condor/ct/warmup_RDM.pt')
            self.load(pth) 
            curr_lr = self.opt.param_groups[0]['lr']
