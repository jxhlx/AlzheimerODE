...
class AddNanMeanAndStdFeature(MapTransformation):
    @validated()
    def __init__(
        self,
        target_field: str,
    ...
    def map_transform(self, data: DataEntry, is_train: bool) -> DataEntry:
        data[self.feature_name] = np.array(
            [np.nanmean(data[self.target_field]), np.nanstd(data[self.target_field])]
        )
...
class AddNanMeanFeature(MapTransformation):
    @validated()
    def __init__(
        self,
        target_field: str,
    ...
    def map_transform(self, data: DataEntry, is_train: bool) -> DataEntry:
        scale = np.array([np.nanmean(data[self.target_field])])
        scale = np.clip(scale, a_min=self.minimum_scale, a_max=np.inf)
        data[self.feature_name] = scale
        return data
