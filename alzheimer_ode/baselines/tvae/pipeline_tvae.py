...
def sample_given_cond(
    ...
    cat_features = list(map(str, range(X_num.shape[1], X_num.shape[1]+X_cat.shape[1]))) if X_cat is not None else []
    ...
    print(synthesizer.decoder)
    trainable_params = sum(p.numel() for p in synthesizer.decoder.parameters())
    ...
    while len(remain_sample_id) > 0:
        ...
        gen_X_cat = gen_data[cat_features].drop('y', axis=1, errors="ignore").values if len(cat_features) else None
