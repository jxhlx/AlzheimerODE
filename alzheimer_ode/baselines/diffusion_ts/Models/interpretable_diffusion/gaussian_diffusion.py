...
class Diffusion_TS(nn.Module):
    ...
    def _train_loss(self, x_start, t, target=None, noise=None, padding_masks=None):
        ...
        if padding_masks is not None:
            # mask: [B, T] -> [B, T, 1]
            mask_expanded = padding_masks.unsqueeze(-1)

            loss_masked = loss_elementwise * mask_expanded

            sample_loss_sum = loss_masked.sum(dim=(1, 2))

            sample_valid_counts = mask_expanded.sum(dim=1).squeeze(-1) * loss_elementwise.shape[-1]

            train_loss = sample_loss_sum / (sample_valid_counts + 1e-8)
        else:
            train_loss = loss_elementwise.mean(dim=(1, 2))
        ...
        if self.use_ff:
            mo_cpu = model_out.transpose(1, 2).contiguous().cpu()
            tg_cpu = target.transpose(1, 2).contiguous().cpu()

            fft1 = torch.fft.fft(mo_cpu, norm='forward')
            fft2 = torch.fft.fft(tg_cpu, norm='forward')

            fft1 = fft1.to(x_start.device)
            fft2 = fft2.to(x_start.device)

            fft1, fft2 = fft1.transpose(1, 2), fft2.transpose(1, 2)

            loss_real = self.loss_fn(torch.real(fft1), torch.real(fft2), reduction='none')
            loss_imag = self.loss_fn(torch.imag(fft1), torch.imag(fft2), reduction='none')
            fl_elementwise = loss_real + loss_imag

            if padding_masks is not None:
                mask_expanded = padding_masks.unsqueeze(-1)
                fl_elementwise = fl_elementwise * mask_expanded
                total_valid = mask_expanded.sum() * fl_elementwise.shape[-1]
                fourier_loss = fl_elementwise.sum() / (total_valid + 1e-8)
            else:
                fourier_loss = fl_elementwise.mean()
