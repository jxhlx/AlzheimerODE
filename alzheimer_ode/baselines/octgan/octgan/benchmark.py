...
from octgan.data import load_atndataset
from octgan.evaluate import compute_scores, sample_metrics
...
def _score_synthesizer_on_dataset(name, synthesizer, iteration, args):
    try:
        ...
        train, test, discrete_columns = load_atndataset(args.dataset_name, benchmark=True)
        if isinstance(synthesizer, type) and issubclass(synthesizer, BaseSynthesizer):
            ...
            synthesizer = synthesizer(dataset_name=args.dataset_name, args=args)

        # synthesized = synthesizer(train, categoricals, ordinals)
        synthesizer.fit(train, discrete_columns)
        # scores = compute_scores(test, synthesized, meta)
        start = time.time()
        scores = sample_metrics(train, test, synthesizer)
        end = time.time()
