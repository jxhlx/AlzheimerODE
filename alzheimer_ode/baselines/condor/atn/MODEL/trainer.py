...
class Dataset1D(Dataset):
    ...
    def collate_fn(self, samples):
        ...
        for sample in samples:
            ...
            mask = torch.ones(n_visit)
            if diff > 0:
                zero_pad_x = torch.zeros(size=(1, diff, 444))
...
def Upsample(idx, dim, dim_out = None):
    if idx == 0:
        return nn.Sequential(
            nn.Upsample(size=(111,), mode = 'nearest'),
...
class Unet1D(nn.Module):
    ...
    def forward(self, x, time, label, age, x_self_cond = None):
        ...
        x = self.mid_attn(x)
        x = self.mid_block2(x, t) # torch.Size([1, 512, 55])
...
class GaussianDiffusion1D(nn.Module):
    def __init__(
        ...
        *,
        ...
        # self.is_ddim_sampling = self.sampling_timesteps < timesteps # 같음 (즉, False)
    ...
    def sample_fromv2(self, label, age, batch_size = 16):
        ...
        for v in range(1, num_visit):
            ...
            sampled_result_from2nd.append(sampled.clone()) # sampled with condition
...
class Trainer1D(object):
    ...
    def train(self):
        ...
        if self.warmup == True:
            ...
            pth = os.path.join('checkpoints/condor/atn/warmup_RDM.pt')
            self.load(pth) 
            curr_lr = self.opt.param_groups[0]['lr']
