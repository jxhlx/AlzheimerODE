...
class GaussianDiffusion1D(nn.Module):
    def __init__(
        ...
        self.OR_model_oasis = or_batch.OrdinalRegression(self.args, 'Tau_combined_v2.csv')
        self.OR_model_adni = or_batch.OrdinalRegression(self.args, 'Tau_combined_v2_adni.csv')
    ...
    def model_predictions(self, x, label, age, datatype, t, get_warmup=False, x_self_cond=None, clip_x_start=False,
        ...
        if get_warmup == True:
            model_output = self.warmup_model.model(x, t, label, age, datatype.unsqueeze(-1), x_self_cond)
        else:
    ...
    def sample(self, label, age, datatype, get_warmup=False, batch_size=65):
        ...
        for v in range(label.shape[1]):
            ...
            sampled = sample_fn(init_data, label[:, v, :], age[:, v], datatype[:, v], get_warmup,
                                (batch_size, channels, num_node))
...
class Trainer1D(object):
    def __init__(
        ...
        train_dl = DataLoader(train_dataset, batch_size=162, collate_fn=train_dataset.collate_fn, shuffle=False,
                              pin_memory=True, num_workers=cpu_count())
            ...
            while self.step < self.train_num_steps:
                ...
                if accelerator.is_main_process:
                    ...
                    if (self.step != 0 and self.step % self.save_and_sample_every == 0) or (
                            self.step == self.train_num_steps - 1):
                        self.ema.eval()
                        ...
                        with torch.no_grad():
                            ...
                            if milestone == 1:
                                ...
                                image = wandb.Image(test_x[0].unsqueeze(0), caption=fn, file_type="png")
                                wandb.run.log({"train sample": [image]})
                            all_samples_list = list(
                                map(lambda n: self.ema.sample(label=test_label, age=test_age, datatype=test_datatype,
                        ...
                        fn = 'sample-' + str(milestone) + '_' + str(round(wd, 3)) + '_' + str(round(js, 3)) + '_' + str(
                            round(rmse, 3)) + '.png'
                        torch.save(all_samples, os.path.join(self.results_folder, fn))
                        image = wandb.Image(all_samples[0].unsqueeze(0), caption=fn, file_type="png")
                        ...
                        with torch.no_grad():
                            ...
                            all_samples_list = list(
                                map(lambda n: self.ema.sample(label=train_label, age=train_age, datatype=train_datatype,
                                                              batch_size=162), batches))
