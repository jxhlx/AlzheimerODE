...
pipeline = {
    'ddpm': METHOD_DIR / 'scripts' / 'pipeline.py',
    'smote': BASELINES_DIR / 'smote' / 'pipeline_smote.py',
    'ctabgan': METHOD_DIR / 'CTAB-GAN' / 'pipeline_ctabgan.py',
    'ctabgan-plus': BASELINES_DIR / 'ctab_gan_plus' / 'pipeline_ctabganp.py',
    'tvae': BASELINES_DIR / 'tvae' / 'pipeline_tvae.py',
}
...
def eval_seeds(
    ...
    with tempfile.TemporaryDirectory() as dir_:
        ...
        for sample_seed in range(n_datasets):
            ...
            if eval_type != 'real':
                subprocess.run([sys.executable, str(pipeline[sampling_method]), '--config', str(dir_ / "config.toml"), '--sample'], check=True)
