...
class OrdinalRegression():
    def __init__(self, args, fn):
        self.df = pd.read_csv(os.path.join(args.dir, fn))
        ...
        if 'oasis' in fn:
            self.datatype = 0
            self.n_thresholds = 1
            self.data = self.df.iloc[:, -74:]
            self.num_node = 74
            self.x_columns = ['Node ' + str(i) for i in range(1, 75)]
        else:
            self.datatype = 1
        ...
        variables = self.df[['age_norm'] + self.x_columns]
        self.model = OrderedModel(self.label, variables, distr='logit')
    ...
    def kde_density(self, data, x_values, mask, bandwidth=0.001):
        ...
        kernel_vals = torch.exp(-0.5 * (diffs / bandwidth_tensor) ** 2) / (
                    bandwidth_tensor * torch.sqrt(torch.tensor(2 * torch.pi, device=data.device)))

        # Apply mask to kernel values
        mask = mask.reshape(mask.size(0), -1, 1)
        masked_kernel_vals = kernel_vals * mask
    ...
    def get_cdf(self, y, a, a_tol):
        ...
        a_expanded = a.unsqueeze(1).to(self.tensor_a.dtype)
        ...
        p_y_given_a = torch.sum(age_mask * label_mask, dim=1).float() / (
                    torch.sum(age_mask, dim=1).float() + self.epsilon)
        ...
        p_x_given_a_y = (p_y_given_x_a.unsqueeze(1) * p_x_given_a) / (
                    p_y_given_a.unsqueeze(1).unsqueeze(2) + self.epsilon)
    ...
    def sample_from_cdf(self, y, a, num_samples=1):
        if self.datatype == 0:
            y = torch.where(y == 4, torch.tensor(1), y)
        ...
        if self.datatype == 0:
            sampled_values = repeat(sampled_values, 'b s n -> b s (n d)', d=2)
