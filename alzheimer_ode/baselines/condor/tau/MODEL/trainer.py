...
class warmup_Dataset1D(Dataset):
    ...
    def load_data(self):
        ...
        for filename in os.listdir(os.path.join(self.dir, self.status + '_data')):
            ...
            assert (len(label) == len(age)) and (
                        data.shape[1] == len(label)), "Length of label, age, data should be same"

            age_ = torch.tensor(age_).type(torch.float32)
            for v in range(n_visit):
                self.age.append(age_[v])

            datatype = torch.load(os.path.join(self.dir, self.status + '_datatype', filename)).type(torch.float32)
            for _ in range(n_visit):
                self.type.append(datatype)

    def collate_fn(self, samples):
        collate_x, collate_y, collate_a, collate_mask, collate_type = [], [], [], [], []
        for sample in samples:
            x, y, a, dt = sample[0], sample[1], sample[2], sample[3]
            n_visit = x.shape[1]
            ...
            collate_type.append(dt)
        X, Y, A, M, D = torch.stack(collate_x), torch.stack(collate_y), torch.stack(collate_a), torch.stack(
            collate_mask), torch.stack(collate_type)
        return X, Y, A, M, D
    ...
    def __getitem__(self, idx):
        ...
        age = self.age[idx]
        datatype = self.type[idx]
        return data, label, age, datatype
...
class Dataset1D(Dataset):
    ...
    def load_data(self):
        ...
        for filename in os.listdir(os.path.join(self.dir, self.status + '_data')):
            ...
            datatype = torch.load(os.path.join(self.dir, self.status + '_datatype', filename))
            datatype = repeat(datatype, 'k -> (k v)', v=len(label))  # k==1
            self.type.append(datatype.type(torch.float32))

    def collate_fn(self, samples):
        collate_x, collate_y, collate_a, collate_mask, collate_dt = [], [], [], [], []
        ...
        for sample in samples:
            ...
            if diff > 0:
                ...
                zero_pad_dt = torch.zeros(size=(diff,))
                ...
                collate_dt.append(torch.cat([dt, zero_pad_dt], dim=0))
                ...
                collate_a.append(a)
                collate_mask.append(mask)
                collate_dt.append(dt)

        X, Y, A = torch.stack(collate_x), torch.stack(collate_y), torch.stack(collate_a)
        M, D = torch.stack(collate_mask), torch.stack(collate_dt)
        return X, Y, A, M, D
...
class Unet1D(nn.Module):
    ...
    def forward(self, x, time, label, age, datatype, x_self_cond=None):
        ...
        '''for single data sampling, reshape data as below'''
        if datatype.shape == torch.Size([1]):
            datatype = datatype.unsqueeze(0)

        datatype = datatype.squeeze(0)
        cemb1 = torch.cat((age_emb1, label_emb1, datatype), dim=-1)
        cemb2 = torch.cat((age_emb2, label_emb2, datatype), dim=-1)  #
        ...
        x = self.init_conv(x.type(torch.float32))
        assert not torch.isnan(x).any().item(), "x has nan"
        r = x.clone()
        ...
        x = self.mid_attn(x)
        x = self.mid_block2(x, t)  # torch.Size([1, 512, 18])
...
class GaussianDiffusion1D(nn.Module):
    def __init__(
        ...
        self.OR_model_oasis = or_batch.OrdinalRegression(self.args, 'Tau.csv')
        self.OR_model_adni = or_batch.OrdinalRegression(self.args, 'Tau.csv')
    ...
    def model_predictions(self, x, label, age, datatype, t, get_warmup=False, x_self_cond=None, clip_x_start=False,
                          rederive_pred_noise=False):
        if get_warmup == True:
            model_output = self.warmup_model.model(x, t, label, age, datatype, x_self_cond)
        else:
            model_output = self.model(x, t, label, age, datatype, x_self_cond)
    ...
    def p_sample(self, x, label, age, t: int, x_self_cond=None, clip_denoised=True):
        ...
        model_mean, _, model_log_variance, x_start = self.p_mean_variance(x=x, label=label, age=age, t=batched_times,
    ...
    @torch.no_grad()
    def ddim_sample_init(self, init_data, label, age, datatype, get_warmup, shape, clip_denoised=True):
        ...
        for time, time_next in tqdm(time_pairs, desc='sampling'):
            ...
            pred_noise, x_start, *_ = self.model_predictions(data, label, age, datatype, time_cond, get_warmup,
    ...
    @torch.no_grad()
    def ddim_sample_fromv2(self, init_data, label, age, datatype, shape, clip_denoised=True):
        batch, device, total_timesteps, sampling_timesteps, eta = shape[
            0], self.betas.device, self.num_timesteps_fromv2, self.num_timesteps_fromv2, self.ddim_sampling_eta,
        ...
        for time, time_next in tqdm(time_pairs, desc='sampling loop time step'):
            ...
            pred_noise, x_start, *_ = self.model_predictions(data, label, age, datatype, time_cond, self_cond,
    ...
    @torch.no_grad()
    def sample(self, label, age, datatype, get_warmup=False, batch_size=65):
        ...
        sampled = sample_fn(init_data, label, age, datatype, get_warmup, (batch_size, channels, num_node))
    ...
    @torch.no_grad()
    def sample_fromv2(self, label, age, datatype, batch_size=16):
        num_node, channels = self.num_node, self.channels
        num_visit = label.shape[1]

        sampled_result_v0, sampled_result_from2nd = [], []

        sampled = self.sample(label[:, 0, :], age[:, 0], datatype[:, 0], True, batch_size)
        ...
        for v in range(1, num_visit):
            ...
            sampled = sample_fn(prev_data, label[:, v, :], age[:, v], datatype[:, v], (batch_size, channels, num_node))
    ...
    def q_sample_between_visits(self, label, age, datatype, t):
        ...
        indices_oasis = torch.where(datatype == 0)[0]
        indices_adni = torch.where(datatype == 1)[0]

        sampled_x = torch.zeros(batch_size, 1, self.num_node).cuda()
        if indices_oasis.shape[0] != 0:
            self.OR_model_oasis.initialize_x_values(indices_oasis.shape[0])
            sampled_x_oasis = self.OR_model_oasis.sample_from_cdf(label_t[indices_oasis], age_t[indices_oasis])
            sampled_x[indices_oasis] = sampled_x_oasis

        if indices_adni.shape[0] != 0:
            self.OR_model_adni.initialize_x_values(indices_adni.shape[0])
            sampled_x_adni = self.OR_model_adni.sample_from_cdf(label_t[indices_adni], age_t[indices_adni])
            sampled_x[indices_adni] = sampled_x_adni
    ...
    def p_losses(self, x_start, label, age, datatype, visit, t, x_diff=None, noise=None):
        if visit == 0:  # first visit
            ...
            x_self_cond = None
            model_out = self.model(x, t, label, age, datatype, x_self_cond)
            ...
            cohort_x_t = self.q_sample_between_visits(label, age, datatype, t)  # x_t is sampled from P(X|Y=y, A=a)
            cohort_x_t_plus_1 = self.q_sample_between_visits(label, age, datatype,
                                                             t + 1)  # x_(t+1) is sampled from P(X|Y=y, A=a)
            ...
            model_out = self.model(x_t, t, label[:, -1, :], age[:, -1], datatype.unsqueeze(-1), x_self_cond)
    ...
    def forward(self, data, label, age, datatype, *args, **kwargs):
        b, c, v, n, device, num_node, = *data.shape, data.device, self.num_node
        assert n == num_node, f'seq length must be {num_node}'

        if v == 1:
            t = torch.randint(0, self.num_timesteps, (b,), device=device).long()
            model_out, noise = self.p_losses(data[:, :, 0, :], label[:, 0, :], age[:, 0], datatype[:, 0], 0, t, *args,
                                             **kwargs)
            return model_out, noise
        else:
            ...
            for i in range(1, v):
                ...
                model_out, noise = self.p_losses(data[:, :, i - 1:i + 1, :], label[:, i - 1:i + 1, :],
                                                 age[:, i - 1:i + 1], datatype[:, i], i + 1, t, x_diff, *args, **kwargs)
...
class Trainer1D(object):
    def __init__(
        ...
        # training
        dl = DataLoader(train_dataset, batch_size=train_batch_size, collate_fn=train_dataset.collate_fn, shuffle=True,
                        pin_memory=True, num_workers=cpu_count())
        dl = self.accelerator.prepare(dl)
        self.dl = cycle(dl)

        # train evaluation for the whole train set
        train_dl = DataLoader(train_dataset, batch_size=73, collate_fn=train_dataset.collate_fn, shuffle=False,
                              pin_memory=True, num_workers=cpu_count())
        self.train_dl = cycle(train_dl)

        # test evalutaion
        test_dl = DataLoader(test_dataset, batch_size=test_size, collate_fn=test_dataset.collate_fn, shuffle=False,
                             pin_memory=True, num_workers=cpu_count())
        ...
        self.model.OR_model_oasis.get_device(self.device)  ### put OR_model onto designated GPUs
        self.model.OR_model_adni.get_device(self.device)
            ...
            while self.step < self.warmup_num_steps:
                ...
                with torch.autograd.detect_anomaly():
                    for _ in range(self.gradient_accumulate_every):
                        data = next(self.dl)
                        x, label, age, mask, datatype = data[0], data[1], data[2], data[3], data[4]

                        with self.accelerator.autocast():
                            model_out, noise = self.model(x[:, :, :].unsqueeze(-2), label[:, :].unsqueeze(-2),
                                                          age[:].unsqueeze(-1), datatype[:].unsqueeze(-1))
                ...
                if accelerator.is_main_process:
                    ...
                    if (self.step != 0 and self.step % self.save_and_sample_every == 0) or (
                            self.step == self.train_num_steps - 1) or (self.step >= 7000 and self.step % 10 == 0):
                        self.ema.eval()
                        ...
                        with torch.no_grad():
                            ...
                            test_x, test_label, test_age, test_mask, test_dt = test_data[0].cuda(), test_data[1].cuda(), \
                            test_data[2].cuda(), test_data[3].cuda(), test_data[4].cuda()

                            if milestone == 1:
                                fn = 'warmup_GT.png'
                                torch.save(test_x, os.path.join(self.results_folder, fn))
                                image = wandb.Image(test_x[0].unsqueeze(0), caption=fn,
                                                    file_type="png")  ## visualize first data
                                wandb.run.log({"test sample": [image]})
                            all_samples_list = list(
                                map(lambda n: self.ema.sample(label=test_label, age=test_age, datatype=test_dt,
                                                              batch_size=self.test_size), batches))
                        all_samples = torch.cat(all_samples_list, dim=0)
                        wd, js, rmse = self.metrics(all_samples, test_x.squeeze())

                        if wd != None:
                            fn = 'warmup_sample-' + str(milestone) + '_' + str(round(wd, 3)) + '_' + str(
                                round(js, 3)) + '_' + str(round(rmse, 3)) + '.png'
                            torch.save(all_samples, os.path.join(self.results_folder, fn))
                            image = wandb.Image(all_samples[0].unsqueeze(0), caption=fn,
                                                file_type="png")  ## visualize first data
                            ...
                            if rmse < rmse_min:
                                ...
                                rmse_jsd_min = js
                                rmse_wd_min = wd
                                fn = 'warmup_RDM-' + str(milestone) + '_' + str(round(rmse_wd_min, 3)) + '_' + str(
                                    round(rmse_jsd_min, 3)) + '_' + str(round(rmse_min, 3)) + '.pt'
                                ...
                                fn = 'warmup_best_sample-' + str(milestone) + '_' + str(round(wd, 3)) + '_' + str(
                                    round(js, 3)) + '_' + str(round(rmse, 3)) + '.pt'
                                torch.save(all_samples, os.path.join(self.results_folder, fn))
                            ...
                            if js < jsd_min:
                                ...
                                jsd_rmse_min = rmse
                                jsd_wd_min = wd
                                fn = 'warmup_RDM-' + str(milestone) + '_' + str(round(jsd_wd_min, 3)) + '_' + str(
                                    round(jsd_min, 3)) + '_' + str(round(jsd_rmse_min, 3)) + '.pt'
                                pth = os.path.join(self.results_folder, fn)
                                self.save(pth)
                                best_jsd_results = {
                                    "best_jsd_rmse": rmse,
                            ...
                            if wd < wd_min:
                                ...
                                wd_jsd_min = js
                                wd_rmse_min = rmse
                                fn = 'warmup_RDM-' + str(milestone) + '_' + str(round(wd_min, 3)) + '_' + str(
                                    round(wd_jsd_min, 3)) + '_' + str(round(wd_rmse_min, 3)) + '.pt'
                                pth = os.path.join(self.results_folder, fn)
                                self.save(pth)
                                best_wd_results = {
                                    "best_jsd_rmse": rmse,
    ...
    def train(self):
        ...
        if self.warmup == True:
            ...
            pth = os.path.join('checkpoints/condor/tau/warmup_RDM.pt')
            self.load(pth)
            curr_lr = self.opt.param_groups[0]['lr']
            ...
            '''Training for TDM'''
            accelerator.print('Training TDM !!!')
                ...
                while self.step < self.train_num_steps:
                    ...
                    with torch.autograd.detect_anomaly():
                        for _ in range(self.gradient_accumulate_every):
                            ...
                            with self.accelerator.autocast():
                                model_out, noise = self.model(x, label, age, datatype)
                    ...
                    if accelerator.is_main_process:
                        self.ema.train()
                        if (self.step > 1000 and self.step % self.save_and_sample_every == 0) or (
                                self.step == self.train_num_steps - 1) or (self.step >= 7000 and self.step % 10 == 0):
                            self.ema.eval()
                            print('Sampling in Epoch #', self.step)
                            ...
                            with torch.no_grad():
                                milestone = self.step // self.save_and_sample_every
                                batches = num_to_groups(self.num_samples, 36)

                                test_data = next(self.test_dl)
                                # torch.Size([65, 1, 6, 148]) torch.Size([65, 6, 5]) torch.Size([65, 6]) torch.Size([65, 6])
                                test_x, test_label, test_age, test_mask, test_datatype = test_data[0].cuda(), test_data[
                                    1].cuda(), test_data[2].cuda(), test_data[3].cuda(), test_data[4].cuda()
                                test_datatype = test_datatype.unsqueeze(-1)
                                if milestone == 1:
                                    fn = 'GT.png'
                                    torch.save(test_x, os.path.join(self.results_folder, fn))
                                    image = wandb.Image(test_x[0].unsqueeze(0), caption=fn,
                                    ...
                                    wandb.run.log({"test sample": [image]})
                                all_samples_list = list(
                                    map(lambda n: self.ema.sample_fromv2(label=test_label, age=test_age,
                            ...
                            fn = 'sample-' + str(milestone) + '_' + str(round(wd, 3)) + '_' + str(
                                round(js, 3)) + '_' + str(round(rmse, 3)) + '.png'
                            torch.save(all_samples, os.path.join(self.results_folder, fn))
                            image = wandb.Image(all_samples[0].unsqueeze(0), caption=fn,
                                                file_type="png")  ## visualize first data
                            ...
                            with torch.no_grad():
                                milestone = self.step // self.save_and_sample_every
                                batches = num_to_groups(self.num_samples, 73)

                                train_data = next(self.train_dl)
                                train_x, train_label, train_age, train_mask, train_datatype = train_data[0].cuda(), \
                                train_data[1].cuda(), train_data[2].cuda(), train_data[3].cuda(), train_data[4].cuda()
                                train_datatype = train_datatype.unsqueeze(-1)
                                all_samples_list = list(
                                    map(lambda n: self.ema.sample_fromv2(label=train_label, age=train_age,
                                                                         datatype=train_datatype, batch_size=73),
                                        batches))
                            ...
                            if rmse < rmse_min:
                                ...
                                if wd < 20:
                                    fn = 'best_model-' + str(milestone) + '_' + str(round(wd, 3)) + '_' + str(
                                        round(js, 3)) + '_' + str(round(rmse, 3)) + '_' + str(
                                        round(train_wd, 3)) + '_' + str(round(train_js, 3)) + '_' + str(
                                        round(train_rmse, 3)) + '.pt'
                                    pth = os.path.join(self.results_folder, fn)
                                    self.save(pth)
                                    fn = 'best_sample-' + str(milestone) + '_' + str(round(wd, 3)) + '_' + str(
                                        round(js, 3)) + '_' + str(round(rmse, 3)) + '_' + str(
                                        round(train_wd, 3)) + '_' + str(round(train_js, 3)) + '_' + str(
                                        round(train_rmse, 3)) + '.pt'
                                    torch.save(all_samples, os.path.join(self.results_folder, fn))
                            ...
                            if js < jsd_min:
                                ...
                                if wd < 20:
                                    fn = 'best_model-' + str(milestone) + '_' + str(round(jsd_wd_min, 3)) + '_' + str(
                                        round(jsd_min, 3)) + '_' + str(round(jsd_rmse_min, 3)) + '.pt'
                                    pth = os.path.join(self.results_folder, fn)
                                    self.save(pth)
                                    fn = 'best_sample-' + str(milestone) + '_' + str(round(wd, 3)) + '_' + str(
